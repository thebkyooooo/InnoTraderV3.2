...
const columnDefs: ColDef<DailyQuoteItem>[] = [
  ...
  { field: 'open',        headerName: '시가',    flex: 1, minWidth: 120, type: 'numericColumn',
    valueFormatter: p => fmtNum(p.value),
    cellStyle: p => { const pc = p.data ? p.data.price - p.data.prevDiff : 0; const c = p.value > pc ? UP : p.value < pc ? DOWN : null; return c ? { color: c } : null } },
...
export function DailyQuoteGrid({ symbol }: DailyQuoteGridProps) {
  ...
  const live = useStockPriceWS(symbol)

  // 최상단(오늘) 행을 실시간 시세로 갱신. 거래일이 같을 때만 덮어쓴다.
  const rows = useMemo(() => {
    if (!live || items.length === 0) return items
    const top = items[0]
    if (live.date && live.date !== top.date) return items
    const todayLive: DailyQuoteItem = {
      ...top,
      price: live.price,
      ...
      low: live.low,
      volume: live.volume,
      turnoverMan: live.tradingAmount,
    }
