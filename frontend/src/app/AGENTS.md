...
## Purpose
Next.js App Router 라우트 정의. 라우트 그룹(`(auth)`, `(dashboard)`, `(trading)`)으로 레이아웃을 분리하고, 각 그룹은 독립적인 layout.tsx를 가진다.
...
| `(dashboard)/` | 대시보드 라우트 그룹 — 사이드바·헤더 레이아웃 (see `(dashboard)/AGENTS.md`) |
| `(trading)/` | 트레이딩 라우트 그룹 — 거래 화면 |

## For AI Agents

### Working In This Directory
- 라우트 그룹 `(name)` 은 URL에 포함되지 않음 — `(dashboard)/dashboard/page.tsx` → `/dashboard`
...
| `/dashboard/portfolio` | `(dashboard)/dashboard/portfolio/page.tsx` |
